...
We study two coupled Kuramoto oscillators with antisymmetric sine coupling and additive noise:
\begin{align}
...
\begin{itemize}
    \item Classical (Bell) bound: $|S| \leq 2$,
    \item Quantum (Tsirelson) bound: $|S| \leq 2\sqrt{2} \approx 2.828$.
\end{itemize}
...
As summarized schematically in Fig.~\ref{fig:regime}, the CHSH landscape of antisymmetrically coupled Kuramoto oscillators organizes into three regimes, determined primarily by coherence and memory. Figure~\ref{fig:landscape} shows a representative three-dimensional phase landscape across coupling $K$, noise $\sigma$, and measurement geometry. The Tsirelson ridge and RUT plateau appear as distinct surfaces embedded in this landscape.
...
\textbf{Mechanism:} antisymmetric coupling maintains echo memory and correlation structure even as noise partially degrades lock. Violations remain robust as long as echo coherence is preserved.
...
The characteristic flatness of the plateau is evident: $|S|$ varies by only about $5\%$ across a fourfold increase in noise. At fixed $\sigma = 0.1$, violations occupy a broad region in $(K,\Delta\omega)$ space, indicating that they are not confined to fine-tuned parameter choices.

E103C, with time-varying coupling and $\sigma = 0.1$, lies clearly on the plateau:
\begin{itemize}
    \item $\mathrm{PLI} \approx 0.95$,
    \item $|S| \approx 2.42$,
    \item violations sustained throughout the time evolution.
\end{itemize}
...
\end{align}
In the locked regime, this structure produces a stable phase difference $\delta = \theta_2 - \theta_1$ determined by the interplay of coupling and frequency mismatch. Under RUT conditions (see the RUT Bell Theorem in E104C/analysis), the resulting correlations admit the approximation
\begin{equation}
...
\medskip\noindent
\textit{One-sentence summary:}
Violations arise from antisymmetric coupling on a coherent echo field; noise only matters once it erases the echoes.
