...
2. [Model and Methods](#section-2-model-and-methods)
   - 2.1 Dynamical Model
...
- phase coherence (synchronization strength)
- frequency mismatch (detuning)
- coupling strength
- external noise
...
### (2) How robust are these high correlations to noise?
...
- a broad ridge of high-$|S|$ configurations,
- a robust optimum near ($\Delta a$, $\Delta b$) ≈ (98°, 82°),
- and $|S|$ values approaching 2.81, close to the Tsirelson limit of $2\sqrt{2} \approx 2.828$.

### (4) How do detuning and synchronization quality interact with CHSH structure?

We discover that:
- moderate frequency mismatch ($\Delta\omega \approx 0.2$) slightly enhances $|S|$,
- very small mismatch yields rigid locking and marginally lower $|S|$,
- large mismatch weakens synchronization and reduces $|S|$.
...
- to identify the structural ingredients required for $|S| > 2$,
...
- $\theta_A(t)$, $\theta_B(t) \in [0, 2\pi)$ are oscillator phases
- $\omega_A$, $\omega_B$ are intrinsic frequencies
- $\Delta\omega = \omega_B - \omega_A$ is the detuning (frequency mismatch)
- K is the symmetric coupling strength
- $\eta_{A,B}(t)$ are independent Gaussian noise terms with $\eta(t) \sim \mathcal{N}(0, \sigma^2)$.
...
## 2.3 Continuous-Variable CHSH Functional
...
- Critical noise σ_c is identified by interpolation where ⟨|S|⟩ crosses 2.3
- Angle optimization (A2) uses fine 1° grid near expected optimum
# Section 3: Results

![Figure 1](figures/fig1_combined.png)
...
### 3.1.1 High-Correlation Regime and Collapse Point
...
### 3.1.2 Scaling Law and Saturation of Noise Robustness
...
![Figure 2](figures/fig2_sigma_c_scaling.png)
...
- High-correlation plateau for $\sigma \lesssim \sigma_c/2$
- Smooth degradation as noise increases
- Sharp drop at the collapse threshold
- Classical saturation at $\langle|S|\rangle \approx 1.6$–1.8 for $\sigma \gtrsim 1.0$
...
![Figure 3](figures/fig3_S_vs_sigma.png)
...
### 3.1.4 Synchronization and CHSH Correlation Relationship
...
- Return to |S| < 2 is not identical to loss of phase coherence,
- The CHSH observable is more sensitive to decoherence than the Kuramoto order parameter r, and
- |S| dropping below 2 marks a stricter criterion than simple synchrony breakdown.
...
1. Clean high CHSH values up to |S| ≈ 2.815 across all coupling strengths
...
### 3.2.1b Angles and Symmetry Considerations
...
- The landscape is smooth and well-behaved, with no additional local maxima detected at the 1° grid resolution employed
...
![Figure 4](figures/fig4_angle_ridge.png)
...
**Small mismatch (Δω ≲ 0.1)**:
- Near-perfect phase locking with minimal exploration
- May produce slightly lower |S| due to reduced phase-space sampling
- However, exact behavior at Δω = 0 requires experimental verification

**Moderate mismatch (Δω ≈ 0.2)**:
- Optimal tension between locking and exploration
- Sufficient coupling to maintain coherence
- Enough frequency difference to generate rich correlation structure

**Large mismatch (Δω ≳ 0.3)**:
- Coupling insufficient to overcome frequency difference
- Correlation amplitude decreases systematically
- Phase coherence remains high (r ≈ 0.998) but CHSH structure weakens
...
### 3.3.3 Phase Coherence Remains Robust Across Δω
...
- The CHSH observable is more sensitive to Δω than the Kuramoto order parameter
- Correlation amplitude and phase coherence again decouple (cf. Experiment A1)

![Figure 5](figures/fig5_delta_omega.png)
...
- Fixed-frequency implementations should target Δω ≈ 0.3 K for maximum |S|
...
2. Dynamical tension enhances correlation amplitude compared to perfect resonance
3. Phase coherence r robustness across Δω confirms decoupling from CHSH structure

This completes the characterization of the high-correlation parameter regime.

---

## 3.4 Temporal Coherence Beyond |S| > 2 (Experiment B1)
...
1. **Ridge** (σ = 0.2): Deep in the high-correlation region
...
![Figure 6](figures/fig6_memory_panel.png)
...
- Past values of S_inst(t) correlate with values 10 time units later
- Temporal coherence of the correlation observable persists beyond the |S| > 2 regime
...
- **Correlation amplitude** (sensitive to noise, collapses at σ_c)
- **Temporal memory** (robust to noise, persists beyond σ_c)

### 3.4.4 Non-Monotonic Behavior of ρ_S
...
- At low noise, rapid phase dynamics may produce decorrelation at the fixed lag τ = 10
- At intermediate noise, slower dynamics could artificially enhance measured autocorrelation at this timescale
- At high noise, phase diffusion timescales may shift, altering the apparent correlation structure
...
- **|S| > 2 does not require perfect memory**: ρ_S can be moderate (0.76) while |S| is large (2.77)
- **Memory does not imply |S| > 2**: ρ_S can be high (0.86) while |S| is classical (1.59)
...
2. CHSH correlations |S| > 2 and temporal memory are distinct dynamical structures
...
![Figure S1](figures/figS1_control_random.png)
...
### 4.1.1 High CHSH Values in Phase-Coherent Regimes
...
### 4.1.2 Collapse of CHSH Correlations with Increasing Noise
...
### 4.1.3 A Robust Optimum in Measurement Geometry
...
### 4.1.4 Temporal Coherence Persists Beyond the |S| > 2 Regime
...
### 4.3.1 Effective Potential and Phase Locking
...
- Δθ(t) remains confined near the potential minimum,
- but fluctuates enough to avoid a degenerate single-value distribution.

### 4.3.2 Noise as a Shaping Mechanism

Noise plays two opposing roles:

**Low noise:**
Phase locking is too rigid; Δθ explores a narrow region, limiting variation across measurement orientations. |S| is high but not maximized.

**Moderate noise:**
Δθ explores more of the curved region around the potential minimum, enhancing the contrast between the four CHSH terms. This creates the high-|S| ridge.

**High noise:**
Noise-induced diffusion overpowers the restoring force; Δθ becomes broad and unstructured, causing |S| to fall below 2.

The collapse threshold σ_c marks the point where diffusion time scales become comparable to locking time scales.

### 4.3.3 Role of Frequency Mismatch
...
### 4.4.1 Two-Oscillator Model

Behavior may differ in larger oscillator networks.

### 4.4.2 CHSH Functional Form
...
### 4.5.7 Experimental Implementations
...
### 4.8.1 Phase-Difference Dynamics and Drift–Diffusion Balance
...
- **low K**, where the restoring force 2K is too weak for the linear approximation to hold and the dynamics explore a broad region of the 2π potential;
- **high K**, where the sin(Δθ) nonlinearity becomes important and the benefit of increasing K saturates.

Both deviations appear clearly in Fig. 2.

### 4.8.2 Measurement Geometry and Correlation Structure
...
- the stationary distribution P(Δθ) is only approximately Gaussian,
- finite noise introduces mild broadening and asymmetric tails, and
- small detuning generates slight skewness in the drift.
...
### 4.8.3 Detuning-Induced Maximum in |S|
...
### 4.8.4 Temporal Memory and the Persistence of ρ_S
...
### Dynamical Systems and Complex Behavior
...
### Toward Experimental Realizations
...
- Phase-locked lasers
- Electrochemical or biochemical oscillators
...
- Build a geometric theory of $|S| > 2$ in classical phase spaces
- Identify universal scaling laws for collapse and memory
- Investigate applications to neural dynamics, analog computing, and hybrid classical–quantum architectures
